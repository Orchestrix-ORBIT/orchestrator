...
The active work and results are tracked in [INTEGRATION_TESTING.md](INTEGRATION_TESTING.md). Core API HTTP, realtime persistence, and live STOMP delivery checks now pass against isolated PostgreSQL. The next work is merging the updated workflow into `dev`, authorizing STOMP subscriptions, cross-service requests, and live Java service checks against Supabase. The Supabase Data API create/read/delete smoke check passed, but it did not exercise the Java services.

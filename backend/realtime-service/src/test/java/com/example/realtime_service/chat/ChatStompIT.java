...
class ChatStompIT {
    ...
    void deliversOnlyToTheMatchingTenantAndStoresEachMessageSeparately() throws Exception {
        ...
        try {
            ...
            first = client.connectAsync(endpoint, new StompSessionHandlerAdapter() {}).get(10, TimeUnit.SECONDS);
            second = client.connectAsync(endpoint, new StompSessionHandlerAdapter() {}).get(10, TimeUnit.SECONDS);
            ...
            assertThat(history(FIRST_TENANT, projectId)).contains("First tenant message")
                    .doesNotContain("Second tenant message");
            assertThat(history(SECOND_TENANT, projectId)).contains("Second tenant message")
    ...
    private String history(String tenant, UUID projectId) throws Exception {
